...
cmd = torch.CmdLine()
cmd:text()
cmd:text('Train an Image Captioning model: only sentences')
cmd:text()
cmd:text('Options')
...
cmd:option('-checkpoint_path', 'checkpoints/model_only_sentences/', 'folder to save checkpoints into (empty = this folder)')
...
local function eval_split(split, evalopt)
  ...
  while true do
    ...
    --local feats = protos.cnn:forward(data.images) --Ananth
    --local expanded_feats = protos.expander:forward(feats) --Ananth
    ...
    --local sth_err = sth_criterion:forward(expanded_feats, data.vectors:cuda()) --Ananth 
    --loss_sth = loss_sth + sth_err    --Ananth
    -------------------------------------------------------------------------------
      --Ananth start  
  ...
  return loss_sum/loss_evals, predictions, lang_stats  --Ananth
end
...
local function lossFun()
  ...
  ---------------------------------------------------------------  
  --print(data.vectors:size())  --Ananth
  --print(data.vectors[1][1])  --Ananth 
  --print(expanded_feats)  --Ananth 
  ...
  -- and lets get out!
  --local losses = { total_loss = loss , sth_loss = sth_err} --Ananth
...
while true do  
  ...
  print(string.format('iter %d total_loss: %f', iter, losses.total_loss))
  ...
  if (iter % opt.save_checkpoint_every == 0 or iter == opt.max_iters) then
    ...
    print(string.format('validation - total loss: %f', val_loss))
    print(lang_stats)
