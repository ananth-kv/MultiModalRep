...
cmd = torch.CmdLine()
cmd:text()
cmd:text('Train -- Image and Sent in alternative epoch')
cmd:text()
cmd:text('Options')

-- Data input settings
cmd:option('-input_h5','coco/coctalk.h5','path to the h5file containing the preprocessed dataset')
cmd:option('-input_json','coco/coctalk.json','path to the json file containing additional info and vocab')
...
cmd:option('-id', '1002', 'an id identifying this run/job. used in cross-val and appended when writing progress files')
...
local function lossFun()
  ...
  if iter%2 == 0 then
      ...
      sth_err = sth_criterion:forward(expanded_feats, data.vectors:cuda()) --Ananth 
      ---------------------------------------------------------------
