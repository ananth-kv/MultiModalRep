...
function DataLoader:__init(opt)
  ...
  -- load in the vectors - Ananth
  --self.sth_vectors = self.h5_file:read('/sth_vectors'):all()
...
function DataLoader:getBatch(opt)
  ...
  local vectors_batch = torch.FloatTensor(batch_size*seq_per_img, 2400)  --Ananth
  ...
  for i=1,batch_size do
    ...
    if ncap < seq_per_img then
      ...
      for q=1, seq_per_img do
       ...
       vectors = self.h5_file:read('/sth_vectors'):partial({ixl, ixl}, {2401,4800}) --Ananth
    else
      ...
      vectors = self.h5_file:read('/sth_vectors'):partial({ixl, ixl+seq_per_img-1}, {2401,4800}) --Ananth      
    end
    local il = (i-1)*seq_per_img+1
    label_batch[{ {il,il+seq_per_img-1} }] = seq
    vectors_batch[{ {il,il+seq_per_img-1} }] = vectors --Ananth
