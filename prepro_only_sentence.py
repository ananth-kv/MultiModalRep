...
import sys
sys.path.insert(0,'/data/Ananth/code/skip-thoughts/')
import skipthoughts


def prepro_captions(_imgs):
    # preprocess all the captions
    imgs = []
    print 'example processed tokens:'
    for i,img in enumerate(_imgs):
        img['processed_tokens'] = []
        text = str(img['captions']).lower().translate(None, string.punctuation).strip().split()
        text = [item for item in text if item.isalnum()]
        #text = str(img['captions'])
        if len(text) <= 50 and len(text) >= 5:
            img['processed_tokens'].append(text)#.split())
            imgs.append(img)
        if i < 10: print img['processed_tokens']
            
    print 'number of captions: %d' % (len(imgs))
    return imgs
...
def encode_captions(imgs, params, wtoi, model):
  """ 
  ...
  #------ANANTH-------Create skip thought vectors ------------
  print('creating vectors....')
  sth_vectors = []
  vectors = []
  for i,im in enumerate(imgs):    
    for c in im['final_captions']:
        input_str = ' '.join(c)
        vectors.append(input_str) # Get all the sentences together
  
  print(len(vectors))
  sth_vectors = skipthoughts.encode(model,vectors) # Encode the vectors
  assert len(sth_vectors) == len(vectors), 'error creating skipthought vectors'
  #-----------------------------------------------------------
  ...
  return L, label_start_ix, label_end_ix, label_length, sth_vectors

def main(params):

  _imgs = json.load(open(params['input_json'], 'r'))
  seed(123) # make reproducible
  shuffle(_imgs) # shuffle the order

  # tokenization and preprocessing
  temp_len = len(_imgs) / 20
  _imgs = _imgs[0:temp_len]  ##Ananth
  print("Size of sentences: ", len(_imgs))
  imgs = prepro_captions(_imgs)
  print("Size of processed  sentences: ", len(imgs))
  ...
  model = skipthoughts.load_model()  # Ananth

  # encode captions in large arrays, ready to ship to hdf5 file
  L, label_start_ix, label_end_ix, label_length, sth_vectors = encode_captions(imgs, params, wtoi, model)

  # create output h5 file
  N = len(imgs)
  f = h5py.File(params['output_h5'], "w")
  f.create_dataset("labels", dtype='uint32', data=L)
  f.create_dataset("sth_vectors", dtype='float32', data=sth_vectors)  # Ananth
  ...
  for i,img in enumerate(imgs):
    # load the image
    I = imread('coco_dataset/val2014/COCO_val2014_000000581929.jpg')
